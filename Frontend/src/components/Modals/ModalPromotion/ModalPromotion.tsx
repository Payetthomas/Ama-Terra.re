...
const ModalPromotion = ({ promotion, products, onClose }: TModalPromotionProps) => {
  ...
  const [form, setForm] = useState<Omit<TPromotion, "id" | "product" | "products">>({
    type: "percentage",
    value: 0,
    start_date: "",
    end_date: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && promotion?.id) {
        await axios.put(`http://localhost:1818/api/promotion/${promotion.id}`, payload);
  ...
  return (
    <section className={styles.addPromotion} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <h2>{isEdit ? "Modifier la promotion" : "Créer une promotion"}</h2>
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          <input
            ...
            placeholder={form.type === "percentage" ? "Pourcentage %" : "Montant fixe"}
            required
          />
